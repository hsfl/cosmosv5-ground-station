...
/*!
 * \file netradio.h
 * \brief Network-attached radio abstraction layer.
 * \ingroup cosmos_ground_station_device
 * \defgroup cosmos_netradio NetRadio
 *
 * Defines the NetRadio interface for controlling radios that expose a network
 * (TCP/UDP) command port rather than a direct serial connection. Provides
 * frequency, mode, and PTT abstractions compatible with the COSMOS
 * antenna-tracking and communication-pass management agents.
 */
